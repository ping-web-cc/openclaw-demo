...
def _execute_task(task_id, trigger_type='scheduled'):
    with _app.app_context():
        ...
        if os.environ.get('DEMO_MODE') == 'true':
            logger.info('Task %d (%s) skipped: DEMO_MODE=true', task.id, task.name)
            return
        ...
        try:
            from app.notifier import notify_task_result
            notify_task_result(
                task.name, log_entry.status, log_entry.duration_secs,
                trigger_type, stderr=log_entry.stderr,
            )
        except Exception as e:
